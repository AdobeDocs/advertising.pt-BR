...
>[!IMPORTANT]
>
>A prática recomendada é usar as mesmas janelas de retrospectiva e modelo de atribuição no Adobe Advertising e [!DNL Analytics]. Trabalhe com sua equipe de conta da Adobe conforme necessário para identificar as configurações atuais e manter as configurações em sincronia.
...
#### Diferentes janelas de pesquisa para rastreamento view-through {#impression-lookback}
...
Normalmente, a maioria das conversões view-through ocorre com rapidez suficiente para que ambos os sistemas atribuam crédito. No entanto, algumas conversões podem ocorrer fora da janela de retrospectiva de impressão do Adobe Advertising, mas dentro da janela de retrospectiva de [!DNL Analytics]; essas conversões são atribuídas ao view-through em [!DNL Analytics], mas não à impressão no Adobe Advertising.

No exemplo a seguir, suponha que um visitante recebeu um anúncio no Dia 1, realizou uma visita de view-through (ou seja, visitou a página de aterrissagem do anúncio sem clicar anteriormente no anúncio) no Dia 2 e converteu no Dia 45. Nesse caso, o Adobe Advertising rastrearia o usuário dos Dias 1 a 14 (usando uma retrospectiva de 14 dias), o [!DNL Analytics] rastrearia o usuário dos Dias 2 a 61 (usando uma retrospectiva de 60 dias) e a conversão no Dia 45 seria atribuída ao anúncio em [!DNL Analytics], mas não no Adobe Advertising.
...
| Atribuição do Adobe Advertising | Atribuição [!DNL Analytics] | Alocação de [!DNL eVar]/[!DNL rVar] |
|--- |--- |--- |
...
Nos relatórios do Adobe Advertising, você pode comparar instâncias a cliques de maneira semelhante usando a métrica &quot;[!UICONTROL EF ID Instances]&quot; em vez de [!UICONTROL AMO ID Instances]:

```
EF ID Instances to Clicks = ([!UICONTROL EF ID Instances] / [!UICONTROL Adobe Advertising Clicks])
```

Embora você deva esperar uma alta taxa de correspondência entre a ID AMO e a ID EF, não espere 100% de paridade porque a ID AMO e a ID EF rastreiam basicamente dados diferentes, e essa diferença pode levar a pequenas diferenças no total [!UICONTROL AMO ID Instances] e [!UICONTROL EF ID Instances]. No entanto, se o total de [!UICONTROL AMO ID Instances] em [!DNL Analytics] diferir de [!UICONTROL EF ID Instances] no Adobe Advertising em mais de 1%, entre em contato com a equipe de conta da Adobe para obter assistência.

Para obter mais informações sobre AMO ID e EF ID, consulte [Adobe Advertising IDs usadas pelo Analytics](ids.md).
...
## Comparando conjuntos de dados no [!DNL Analytics for Advertising] com o no Adobe Advertising

A [ID do AMO](ids.md) (parâmetro de cadeia de caracteres de consulta s_kwcid) é usada para relatórios no [!DNL Analytics], e a [ID do EF](ids.md) (parâmetro de cadeia de caracteres de consulta ef_id) é usada para relatórios no Adobe Advertising. Como são valores distintos, é possível que um valor seja corrompido ou não seja adicionado à landing page.
...
Felizmente, embora problemas como esse sejam comuns, eles normalmente não resultam em uma alta porcentagem de discrepância. No entanto, se você notar uma grande discrepância entre as IDs AMO no [!DNL Analytics] e as IDs EF no Adobe Advertising, entre em contato com a equipe de conta da Adobe para obter assistência.
...
### Uso de métricas de tráfego do Adobe Advertising para dimensões que não são da Adobe Advertising

A Adobe Advertising fornece ao Analytics [métricas de tráfego específicas de anúncios e as dimensões relacionadas de [!DNL DSP] e [!DNL Search, Social, & Commerce]](advertising-metrics-in-analytics.md). As métricas fornecidas pela Adobe Advertising são aplicáveis somente às dimensões do Adobe Advertising especificadas, e os dados não estão disponíveis para outras dimensões em [!DNL Analytics].

Por exemplo, se você exibir as métricas [!UICONTROL Adobe Advertising Clicks] e [!UICONTROL Adobe Advertising Cost] por Conta, que é uma dimensão do Adobe Advertising, o total de [!UICONTROL Adobe Advertising Clicks] e [!UICONTROL Adobe Advertising Cost] será mostrado por conta.

![Exemplo de métricas do Adobe Advertising em um relatório usando uma dimensão do Adobe Advertising](/help/integrations/assets/a4adc-traffic-supported-dimension.png)

No entanto, se você exibir as métricas [!UICONTROL Adobe Advertising Clicks] e [!UICONTROL Adobe Advertising Cost] por uma dimensão na página (como Página), para a qual o Adobe Advertising não fornece dados, o [!UICONTROL Adobe Advertising Clicks] e o [!UICONTROL Adobe Advertising Cost] para cada página serão zero (0).

![Exemplo de métricas do Adobe Advertising em um relatório usando uma dimensão sem suporte](/help/integrations/assets/a4adc-traffic-unsupported-dimension.png)

### Usando [!UICONTROL AMO ID Instances] como substituto para cliques com dimensões que não sejam do Adobe Advertising
...
>* [Por que os dados do canal podem variar entre o Adobe Advertising e o  [!DNL Marketing Channels]](/help/integrations/analytics/marketing-channels/mc-data-variances.md)
