...
## Requisitos para relatórios de dados do Adobe Advertising em [!DNL Analytics]
...
## Dados do [!DNL Analytics] no Adobe Advertising {#lookback-a4adc}
...
* Janela de retrospectiva de clique na atribuição do Adobe Advertising: o número de dias após o primeiro clique em que o clique pode ser atribuído a uma conversão. Por padrão, esse valor é 60 dias; o máximo é 90 dias
* Janela de retrospectiva de impressão de atribuição do Adobe Advertising: o número de dias depois que ocorre uma impressão de anúncio em que a impressão pode ser atribuída a uma conversão. Por padrão, esse valor é 14 dias; o máximo é 30 dias

  >[!NOTE]
  >
  > A janela de retrospectiva de impressão é específica do Adobe Advertising, não [!DNL Analytics for Advertising], relatórios.

O JavaScript [!DNL Analytics for Advertising] usa essas configurações para determinar até que ponto considerar uma entrada de view-through ou de click-through para o site como válida. Para obter mais informações sobre como view-throughs e click-throughs são determinados, consulte &quot;[Adobe Advertising IDs usadas pelo Analytics](ids.md).&quot;

## Dados do Adobe Advertising em [!DNL Analytics]

[!DNL Analytics] define Adobe Advertising IDs (AMO IDs) na ocorrência do Analytics, sujeita à configuração de persistência [!DNL eVar] do anunciante, que se aplica a click-throughs e view-throughs. A configuração de persistência está definida no back-end do Adobe Advertising e sua equipe de conta da Adobe pode alterá-la.
...
Entre em contato com a equipe de conta da Adobe para obter os ambientes de anúncios mais recentes compatíveis em cada canal.

## O que você deve saber antes da implementação

* A equipe de implementação do Adobe Advertising configura a integração.

* Não serão cobrados custos adicionais por essa integração, nem as chamadas do servidor resultarão em [!DNL Analytics] ou tarifas adicionais do Adobe Advertising.
...
* A integração transmite somente eventos padrão e personalizados do [!DNL Analytics] para a Adobe Advertising para otimização de oferta de mídia paga e esforços de publicidade subsequentes. Ele não passa [!DNL Analytics] segmentos, métricas calculadas e [!DNL eVars] para o Adobe Advertising para otimização de oferta.
...
* O rastreamento de conversão do [!DNL Analytics for Advertising] no Adobe Analytics usa uma janela de retrospectiva de rastreamento configurável (60 dias por padrão). Os relatórios do Adobe Advertising refletem as conversões e o envolvimento do site até o final desta janela de retrospectiva de rastreamento.
