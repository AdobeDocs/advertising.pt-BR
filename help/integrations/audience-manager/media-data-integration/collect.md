---
title: Coletar dados de clique e impressão de campanhas do Advertising DSP
...
Este documento explica como marcar anúncios do Advertising DSP para capturar eventos de impressão e clique baseados em cookies usando pixels do Audience Manager e tarefas adicionais necessárias para usar os dados.
...
## Etapa 2: implementar pixels de evento de impressão e clique em páginas da Web {#implement-dsp-pixels}

Os anunciantes podem criar e implementar tags de evento para suas próprias marcas. Se necessário, entre em contato com o consultor da Adobe Audience Manager ou com a equipe de conta da Adobe para obter suporte.
...
* `${TM_CAMPAIGN_ID_NUM}` é a ID numérica da campanha no DSP. Se você quiser codificar uma ID de campanha individual em vez de usar a macro do DSP, localize a ID nas configurações da campanha.
...
>[!TIP]
>
>* A prática recomendada é incluir a campanha, a inserção, o criativo (anúncio) e as IDs do site para que você possa usar os atributos da campanha para criar características do Audience Manager.
>* Para criar relatórios do Audience Optimization, são necessários parâmetros adicionais.
>* Nos pares de valores chave, substitua os valores pelas [macros do DSP](/help/dsp/campaign-management/macros.md) relevantes para que você possa usar um único pixel em todos os anúncios de todas as campanhas. Por exemplo, altere `d_campaign=[%campaignID%]` para `d_campaign=${TM_CAMPAIGN_ID_NUM}` para capturar IDs de campanha geradas pela macro do Adobe Advertising.
...
## Etapa 3: tarefas de pós-implementação

Depois que as tags de evento são implementadas, os dados fluem para os servidores de coleta de dados da Audience Manager. Conclua as seguintes tarefas antes de usar os dados nos relatórios.
