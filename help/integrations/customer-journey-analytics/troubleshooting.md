...
## Lista de todos os problemas em potencial

| Problema | Mais informações |
| ------- | ---------------- |
| Nenhuma chamada alloy() está visível na guia [!DNL Network] da ferramenta de inspeção de código do seu navegador. | Consulte a seção &quot;[Problemas de instalação e instalação](#issues-installation-setup)&quot; > &quot;[A extensão WebSDK não inicializa](#websdk-extension-doesn't-initialize)&quot; |
...
| Nenhuma solicitação de interação ou coleta é feita para edge.adobedc.net. | Consulte &quot;[Problemas de instalação e configuração](#issues-installation-setup)&quot; > &quot;[A extensão WebSDK não inicializa](#websdk-extension-doesn't-initialize)&quot; |
| As solicitações chegam ao Adobe Experience Platform Edge Network, mas retornam erros 400 ou 500. | Consulte a seção &quot;[Problemas de instalação e instalação](#issues-installation-setup)&quot; > &quot;[Sequência de dados não configurada ou configurada incorretamente](#datastream-not-configured-or-misconfigured)&quot; |
| Nenhum dado é exibido nos relatórios do Adobe Analytics ou do Adobe Advertising. | Consulte a seção &quot;[Problemas de instalação e instalação](#issues-installation-setup)&quot; > &quot;[Sequência de dados não configurada ou configurada incorretamente](#datastream-not-configured-or-misconfigured)&quot; |
...
| O depurador mostra que as condições da regra não são atendidas. | Consulte a seção &quot;[Regras ou eventos não estão sendo acionados](#rules-or-events-don't-fire)&quot; |
| A ação [!UICONTROL Send Event] nunca é executada. | Consulte a seção &quot;[Regras ou eventos não estão sendo acionados](#rules-or-events-don't-fire)&quot; |
...
* Nenhuma chamada alloy() está visível na guia [!DNL Network] da ferramenta de inspeção de código do seu navegador.
* Erro do console: liga não definida.
* Nenhuma solicitação de interação ou coleta é feita para edge.adobedc.net.

#### Possíveis causas e verificação/resolução

| Causa | Correção |
| ----- | --- |
| Biblioteca não publicada ou em estado de rascunho | Vá para [Fluxo de Publicação](https://experienceleague.adobe.com/pt-br/docs/experience-platform/tags/publish/publishing-flow) e verifique se a biblioteca que contém a extensão WebSDK está no estado aprovado/publicado. |
| O código de inserção está ausente ou no ambiente incorreto | Verifique se o [[!DNL Tags] código incorporado](https://experienceleague.adobe.com/pt-br/docs/experience-platform/tags/publish/environments/environments) na página da Web faz referência ao ambiente correto (Dev/Stage/Prod). Procure o ambiente na tag `<head>` para a tag de script `//assets.adobedtm.com/...`. |
| Conflito de carga assíncrono vs. síncrono | Verifique se apenas um [[!DNL Tags] código incorporado](https://experienceleague.adobe.com/pt-br/docs/experience-platform/tags/publish/environments/environments) está presente por página da Web. Códigos incorporados duplicados causam condições de corrida. |
| Bloqueio da política de segurança de conteúdo (CSP) | Adicione `edge.adobedc.net` `and assets.adobedtm.com` às suas [diretivas CSP `connect-src` e `script-src`](https://experienceleague.adobe.com/pt-br/docs/experience-platform/collection/use-cases/configuring-a-csp). |

### Sequência de dados não configurada ou configurada incorretamente {#datastream-not-configured-or-misconfigured}

#### Problemas:

* As solicitações chegam ao Adobe Experience Platform Edge Network, mas retornam erros 400 ou 500.
* Nenhum dado é exibido nos relatórios do Adobe Analytics ou do Adobe Advertising.
...
| Os serviços de sequência de dados não estão habilitados para a propriedade de tag. | [Abra as configurações da sequência de dados](https://experienceleague.adobe.com/pt-br/docs/experience-platform/datastreams/configure) e verifique se os seguintes serviços estão habilitados:<ul><li>Adobe Advertising (para conversão/sincronização de público)</li><li>Adobe Experience Platform (para assimilação de perfil)</li></ul> |
| Incompatibilidade de sandbox | Verifique se a sequência de dados pertence à mesma [sandbox da Adobe Experience Platform](https://experienceleague.adobe.com/pt-br/docs/experience-platform/sandbox/home) que o esquema e o conjunto de dados. Um erro comum é criar um fluxo de dados na sandbox de produção, mas apontar esquemas para a sandbox de desenvolvimento. |
...
| Causa | Correção |
| ----- | --- |
| Os cookies de terceiros estão bloqueados. | Migre para a coleção de dados CNAME próprios [configurando uma ID de cookie própria na configuração do Edge Network da sequência de dados](https://experienceleague.adobe.com/pt-br/docs/experience-platform/datastreams/configure). |
...
* A regra foi salva e incluída na [build de biblioteca](https://experienceleague.adobe.com/pt-br/docs/experience-platform/tags/publish/builds) ativa.
...
* As condições da regra não são muito restritivas. Teste [removendo condições](https://experienceleague.adobe.com/pt-br/docs/experience-platform/tags/ui/rules) temporariamente para isolar o problema.
* A ordem da regra está correta. Se várias regras compartilharem o mesmo evento, verifique a [ordem das regras](https://experienceleague.adobe.com/pt-br/docs/experience-platform/tags/ui/rules).
...
| Causa | Correção |
| ----- | --- |
| As alterações não foram adicionadas a uma biblioteca. | No fluxo de trabalho de publicação, confirme se suas alterações foram adicionadas a uma [biblioteca](https://experienceleague.adobe.com/pt-br/docs/experience-platform/tags/publish/libraries) no ambiente de desenvolvimento. Se você [fizer alterações na biblioteca](https://experienceleague.adobe.com/pt-br/docs/experience-platform/tags/publish/libraries#manage-library-changes), adicione os recursos e salve e crie a biblioteca. |
...
| O código incorporado destina-se ao ambiente errado. | Confirme se o código incorporado na página é do ambiente correto. Por exemplo, se você estiver testando o comportamento de produção, confirme se o código de inserção é o código de inserção de produção. |
| A build da biblioteca falhou silenciosamente. | No fluxo de trabalho de publicação, verifique se a biblioteca mostra um estado [!UICONTROL Build Failed]. Abra a biblioteca e revise o log de compilação; causas comuns de falhas de compilação são configurações de regra inválidas ou conflitos de versão de extensão. |
...
| Causa | Correção |
| ----- | --- |
| O grupo de campos [!UICONTROL Advertising] está ausente do esquema. | <ol><li>Vá para Adobe Experience Platform > [!UICONTROL Data Management] > [!UICONTROL Schemas].</li><li>Abra o esquema usado pelo seu fluxo de dados.</li><li>No painel [!UICONTROL Field Groups], confirme se a **Extensão completa do ExperienceEvent da Adobe Advertising Cloud** está listada.</li><li>Se estiver ausente, selecione **Adicionar**, pesquise por **Adobe Advertising Cloud**, selecione **Extensão completa do Adobe Advertising Cloud ExperienceEvent** e salve as configurações.</li></ol>A republicação da biblioteca [!DNL Tags] não é necessária somente para alterações de esquema, mas você deve remapear o elemento de dados XDM em [!DNL Tags] se novos campos forem adicionados. |
| Os campos obrigatórios do Adobe Advertising estão ausentes no esquema. | Verifique se os campos obrigatórios do Adobe Advertising estão presentes no [esquema](https://experienceleague.adobe.com/pt-br/docs/platform-learn/implement-web-sdk/initial-configuration/configure-schemas) em `_experience.adcloud.conversionDetails`. Consulte &quot;[Referência: campos de esquema obrigatórios](#required-schema-fields)&quot;.<br><br>Se um dos campos estiver ausente, confirme se o grupo de campos **Extensão completa do Adobe Advertising Cloud ExperienceEvent** foi salvo no esquema e atualize o editor de esquema. |
| O URL da página de aterrissagem não inclui os parâmetros de consulta necessários. | Certifique-se de que o URL da landing page inclua os parâmetros de consulta necessários. Em um click-through de anúncio, a URL da página de aterrissagem deve conter a consulta dos parâmetros `s_kwcid` e `ef_id`, como `https://www.example.com/landing-page?s_kwcid=AL!12345!3!abc123&ef_id=abc123xyz:G:s`. Consulte &quot;[Referência: parâmetros de consulta ausentes](#missing-query-parameters)&quot; para obter as causas prováveis. |
| Alguns parâmetros na carga XDM estão ausentes ou vazios. | Para validar a carga XDM de saída, abra a guia Adobe Experience Platform Debugger ou [!DNL Network] da ferramenta de inspeção de código do seu navegador, filtre por `edge.adobedc.net` e inspecione o corpo da solicitação de interação (consulte a carga de exemplo abaixo).<br><br>Se `trackingCode` ou `trackingIdentity` estiverem vazios ou ausentes, a) o parâmetro de consulta não estava presente na página quando a regra foi acionada (verifique a URL e o tempo de evento da regra) ou b) o grupo de campos está ausente do esquema (revise a primeira linha acima). |

##### Referência: campos de esquema obrigatórios {#required-schema-fields}
...
##### Referência: parâmetros de consulta ausentes {#missing-query-parameters}
...
| `ef_id` | O URL da página de aterrissagem não está usando um redirecionamento rastreado pela Adobe Advertising ou o anexamento da ID de EF não está habilitado nas configurações da campanha. |

**Exemplo de carga de click-through**
...
## Relatórios de problemas no Customer Journey Analytics Workspace

### Relatório de resumo

| Problema | Verificação e resolução |
| ----- | --- |
| Nenhum dado de relatório de resumo está disponível no Customer Journey Analytics para Advertising DSP ou Advertising Search, Social e Commerce. | <ol><li>Confirme se o Customer Journey Analytics Workspace está fazendo referência à visualização de dados correta.</li><li>Confirme se o feed do Adobe Advertising para o Customer Journey Analytics está ativado. Verifique com a equipe de conta da Adobe.</li><li>Confirme se a dimensão/classificação/conjunto de dados de pesquisa do Adobe Advertising e o conjunto de dados de resumo estão incluídos na conexão do Customer Journey Analytics.</li><li>Confirme se suas dimensões e métricas de resumo do Adobe Advertising estão incluídas na visualização de dados do Customer Journey Analytics.</li></ol>Se você verificar todas as configurações acima, mas ainda não visualizar os dados de resumo, abra um [tíquete de suporte](https://experienceleague.adobe.com/home?lang=pt-BR&support-tab=home#support) para sua organização. |
| Os dados de relatórios de resumo estão disponíveis no Customer Journey Analytics para o Anunciante 1, mas não para o Anunciante 2. | <ol><li>Confirme se o feed do Adobe Advertising para o Customer Journey Analytics está ativado para o Anunciante 2. Verifique com a equipe de conta da Adobe.</li><li>Confirme se a configuração &quot;[!UICONTROL Backfill all existing data]&quot; está habilitada para seus três conjuntos de dados (dimensão/classificação/pesquisa, resumo e métricas de evento) na conexão do Customer Journey Analytics.</li></ol>Se você verificar todas as condições acima, mas ainda não visualizar os dados de resumo, abra um [tíquete de suporte](https://experienceleague.adobe.com/home?lang=pt-BR&support-tab=home#support) para sua organização. |
| (Usuários do Search, Social e Commerce) Os dados de relatórios de resumo estão disponíveis no Customer Journey Analytics para uma conta do [!DNL Google Ads], [!DNL Meta Ads] ou [!DNL Microsoft Advertising], mas não para outra conta. | Verifique se o feed do Adobe Advertising para o Customer Journey Analytics está ativado para a conta de rede de anúncios específica. Verifique com a equipe de conta da Adobe.<br><br>Se o feed estiver habilitado para uma conta, mas você ainda não visualizar os dados de resumo, abra um [tíquete de suporte](https://experienceleague.adobe.com/home?lang=pt-BR&support-tab=home#support) para a sua organização. Inclua o [!UICONTROL Account ID] para a conta de rede de publicidade. |
| Os dados de relatórios de resumo no Customer Journey Analytics Workspace são diferentes dos dados no Advertising DSP ou Advertising Search, Social e Commerce, ou os dados de resumo estão ausentes em algumas campanhas e entidades de campanha. | <ol><li>Confirme se você está usando os mesmos intervalos de datas no [!DNL Workspace] e no relatório do Adobe Advertising.</li><li>Confirme se os filtros e segmentos aplicados em [!DNL Workspace] e no relatório do Adobe Advertising não estão causando diferenças nos dados.</li><li>Confirme se o [!UICONTROL Time Zone] da sua visualização de dados do Customer Journey Analytics corresponde ao [!UICONTROL Default Timezone] da sua [conta do Advertising DSP](/help/dsp/admin/user-own-profile-edit.md).</li><li>Confirme se a configuração &quot;[!UICONTROL Backfill all existing data]&quot; está habilitada para seus três conjuntos de dados (dimensão/classificação/pesquisa, resumo e métricas de evento) na conexão do Customer Journey Analytics.</li></ol>Se tiver certeza de uma discrepância de dados, abra um [tíquete de suporte](https://experienceleague.adobe.com/home?lang=pt-BR&support-tab=home#support) para sua organização. Inclua o [!UICONTROL Account ID] para a conta de rede de publicidade. Para mostrar evidências da discrepância, inclua capturas de tela e planilhas. Sua equipe de conta da Adobe pode corrigir retroativamente o feed de dados para resolver a discrepância, se necessário. |

### Relatórios no nível do evento

| Problema | Verificação e resolução |
| ----- | --- |
| Os dados de conversão (como `Page Views`) não estão disponíveis para uma dimensão de relatório (como `Campaign`) no Customer Journey Analytics Workspace. | Verifique o seguinte, começando pelos itens com menos barreiras de verificação:<ul><li>Confirme se você está usando a visualização de dados correta.</li><li>Confirme se as métricas de conversão aplicáveis são eventos da Web/online, que o Adobe Advertising pode atribuir às dimensões.</li><li>Confirme se o Adobe Advertising está rastreando click-throughs e view-throughs no site aplicável.</li><li>Na conexão Customer Journey Analytics do conjunto de dados de classificações, confirme se os valores das configurações [!DNL Key] e [!DNL Matching Key] estão corretos: [!DNL Key]: `Tracking Code` (_customername.adLens2.trackingCode), [!DNL Matching Key]: `Tracking Code` (event._experience.adcloud.conversionDetails.trackingCode).</li><li>Confirme se o serviço [!DNL Adobe Advertising] foi adicionado à sequência de dados do Adobe Experience Platform, se o esquema mapeado para a sequência de dados é `XDM ExperienceEvent Schema` e se o grupo de campos `Adobe Advertising Cloud ExperienceEvent Full Extension` foi adicionado ao esquema `XDM ExperienceEvent`.</li><li>Confirme se as configurações do Adobe Advertising estão definidas corretamente na extensão WebSDK e publicadas.</li></ul>Se você verificar todas as configurações acima, mas ainda não visualizar os dados de conversão, abra um [tíquete de suporte](https://experienceleague.adobe.com/home?lang=pt-BR&support-tab=home#support) para sua organização. Inclua o [!UICONTROL Account ID] para a conta de rede de publicidade. |

## Ferramentas úteis de validação e depuração
...
### Guia [!DNL Network] da ferramenta de inspeção de código do navegador

Use a guia [!DNL Network] da ferramenta de inspeção de código do seu navegador (geralmente chamada de &quot;[!DNL Inspect]&quot;) para fazer o seguinte:
...
## Lista de verificação de referência rápida antes de solicitar suporte
...
* A Adobe Experience Platform Debugger confirma que os eventos estão chegando à Edge Network.
...
* O Adobe Experience Platform Debugger confirma que `conversionDetails` está preenchido na carga de saída.
