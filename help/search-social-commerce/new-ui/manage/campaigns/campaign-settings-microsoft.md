...
* *[!UICONTROL Manual CPC]*: (Comprando campanhas para marcas e [!DNL Microsoft Store Ads] campanhas) Usa o modelo de custo por clique (CPC). Para alguns tipos de anúncios, você pode permitir que a rede de anúncios altere ofertas para a campanha:
...
**[!UICONTROL Local Inventory Ads]:** (Somente campanhas de rede de compras) Permite que a rede de anúncios adicione automaticamente as informações do inventário local aos seus anúncios de compras.

**Dica:** se você usar essa configuração, não exclua anúncios locais na configuração [!UICONTROL Inventory Filter].

**Observação:** os anúncios de estoque local exigem dois feeds adicionais para a conta da central de comércio — um com os dados do produto local e outro com o estoque de produto local. Consulte a documentação do [!DNL Microsoft Advertising] para obter mais informações sobre [anúncios de compras locais](https://help.ads.microsoft.com/#apex/ads/en/60055).
